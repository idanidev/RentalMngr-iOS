...
extension Error {
    ...
    ///
    /// Delega en `UserFacingError`, que traduce los fallos técnicos a algo
    /// accionable. Antes esto devolvía el texto crudo de Supabase y el usuario
    /// leía cosas como "Could not find the function ... in the schema cache".
    var safeUserMessage: String {
        if let appError = self as? AppError {
            return appError.errorDescription
                ?? String(localized: "Ha ocurrido un error", locale: LanguageService.currentLocale)
        }
        return UserFacingError(self).message
    }

    /// True si el error viene de un límite del plan: la interfaz puede ofrecer Premium.
    var isPlanLimit: Bool { planLimitUserMessage != nil }
}
